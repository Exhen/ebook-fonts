...
# -*- coding: utf-8 -*-
"""
递归扫描目录及所有子目录中的 .ttf 文件，按「出版方」分子文件夹整理，
文件名格式为：出版方_字体中文名_简繁标签.ttf

- 出版方：name 表 + 路径匹配 _PUBLISHER_RULES 通俗名；规则见脚本内常量。
- 字体名称：优先使用 name 表中含汉字的名称（全名/字族等，按 Windows 中文语言
  优先）；若无中文记录则退回原先的英文优先全名。
- 简繁标签：仍由 classify_han_variant 得到（简体、繁体、简繁通用、非中文、未分类），
  写入文件名，不再单独建简繁子目录。

最终路径：根目录 / 出版方文件夹 / 出版方_中文名_简体.ttf

默认处理仓库根目录（scripts 的上一级）；也可传入目标目录路径。
...
_PUBLISHER_RULES: list[tuple[re.Pattern[str], str]] = [
    # 用户举例与常见大陆字库（可自行增补）
    (re.compile(r"滙文|匯文|汇文|huiwen", re.I), "汇文"),
    ...
    (re.compile(r"\bmaple\b|maple\s*mono", re.I), "Maple"),
    (re.compile(r"小赖|小賴|xiaolai|xiao\s*lai", re.I), "小赖"),
    (
        re.compile(
            r"寒蝉|寒蟬|chilltype|chill\s*type|hanchan|han\s*chan",
            re.I,
        ),
        "寒蝉",
    ),
    (re.compile(r"有梦|有夢|youmeng|you\s*meng", re.I), "有梦"),
    (
        re.compile(
            r"铁蒺藜|鐵蒺藜|tiejili|tie\s*ji\s*li",
            re.I,
        ),
        "铁蒺藜",
    ),
    (re.compile(r"煮豆|zhudou|zhu\s*dou", re.I), "煮豆"),
    (
        re.compile(
            r"\bLXGW\b|落霞孤鹜|霞鹜|lxgwwenkai|luoxia\s*gu\s*wu|LXGW",
            re.I,
        ),
        "落霞孤鹜",
    ),
    (
        re.compile(r"PingFang|蘋方|苹方|ping\s*fang|苹果\s*苹方", re.I),
        "苹果",
    ),
    (
        re.compile(
            r"TrionesType|\bTriones\b|trionestype|璇璣|璇玑",
            re.I,
        ),
        "璇璣",
    ),
    ...
    (re.compile(r"阿里妈妈|阿里巴巴|alibaba|alimama|钉钉", re.I), "阿里"),
    (re.compile(r"\bkalosans\b|kalo\s*sans", re.I), "KaloSans"),
    (
        re.compile(
            r"\bslidefont\b|slide\s*font|演示字体",
            re.I,
        ),
        "演示",
    ),
    ...
    (re.compile(r"文鼎|\barphic\b", re.I), "文鼎"),
    (re.compile(r"辰宇|chenyu|chen\s*yu", re.I), "辰宇"),
    (re.compile(r"\bOPPO\b", re.I), "OPPO"),
    (re.compile(r"仓迹|倉跡|cangji|cang\s*ji", re.I), "仓迹"),
    (re.compile(r"\bvivo\b", re.I), "vivo"),
    (re.compile(r"HONOR|Honor|荣耀"), "HONOR"),
    (re.compile(r"小米|\bxiaomi\b|\bredmi\b", re.I), "小米"),
...
def classify_han_variant(tt: TTFont) -> str:
    """
    根据 cmap 简繁异形字对与 name 表语言 ID，返回用于文件名后缀的标签。
    """
...
def _has_cjk(s: str) -> bool:
    return bool(_HAS_CJK_RE.search(s))


# Windows name 语言 ID：中文地区优先序（越小越优先）
_WIN_LANG_CHINESE_ORDER: tuple[int, ...] = (
    0x0804,  # 简体中文（中国）
    0x1004,  # 简体中文（新加坡）
    0x0404,  # 繁体中文（台湾）
    0x0C04,  # 繁体中文（香港）
...
def _win_lang_sort_key(lang_id: int) -> int:
    lid = lang_id & 0xFFFF
    try:
        return _WIN_LANG_CHINESE_ORDER.index(lid)
    except ValueError:
        return 100 + lid


def pick_chinese_display_name(tt: TTFont) -> str | None:
    """
    从 name 表选取含汉字的显示名：优先 Windows 下中文语言的全名 (4)，
    其次 Typographic Family (16)、Family (1)；含 Mac 表中含汉字的 4/1/16。
    """
    tab = tt.get("name")
    if not tab or not tab.names:
        return None
    ...
    for rec in tab.names:
        ...
        if not s or not _has_cjk(s):
            continue
        plat_pri = 0 if rec.platformID == 3 else 1
        if rec.platformID == 3:
            lang_pri = _win_lang_sort_key(rec.langID)
        else:
            lang_pri = 30
        nid_pri = {4: 0, 16: 1, 1: 2}.get(rec.nameID, 9)
        scored.append(((plat_pri, lang_pri, nid_pri), s))
...
def build_ttf_stem(pub: str, font_cn: str, variant: str, max_stem: int = 180) -> str:
    """出版方_字体名_简繁，段内空白变下划线，合并连续下划线。"""
    parts: list[str] = []
    for raw in (pub, font_cn, variant):
        t = INVALID_CHARS.sub("_", raw.strip())
        t = t.replace(".", "_")
        t = re.sub(r"\s+", "_", t).strip("_")
        parts.append(t or "未命名")
    stem = "_".join(parts)
    ...
    for path in ttf_files:
        ...
        try:
            with TTFont(path, fontNumber=0) as tt:
                display_cn = pick_chinese_display_name(tt)
                display = display_cn or pick_preferred_name(tt)
        ...
        dest_parent = (directory / publisher_folder).resolve()
        font_stem_for_file = build_ttf_stem(pub_label, display, variant_raw)
        final_name = sanitize_filename(f"{font_stem_for_file}.ttf")
        bucket = used_targets[dest_parent]
        new_path = unique_path(dest_parent / final_name, bucket, same_as=path)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="按出版方分目录，文件名：出版方_中文名_简繁.ttf"
    )
    parser.add_argument(
        "directory",
        nargs="?",
        default=None,
        help="含 TTF 的目录（默认：仓库根目录）",
    )
    parser.add_argument(
        "-n",
        "--dry-run",
    ...
    base = Path(args.directory) if args.directory else repo_root
